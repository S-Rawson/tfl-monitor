...
# Helper functions for fetching and formatting Tube/Bus data
...
async def _get_list_modes():
    ...
    all_modes = requests.get("https://api.tfl.gov.uk/Line/Meta/Modes")
    all_modes_clean = json.loads(all_modes.text)
    all_modes_list = []
    # Consider using a comprehension for clarity and efficiency
...
async def _all_valid_routes_all_lines(modes):
    ...
    all_lines_routes = requests.get(
        f"https://api.tfl.gov.uk/Line/Mode/{modes}/Route?serviceTypes={service_type}"
    )
...
async def _all_valid_routes_single_line(line):
    ...
    service_type = "Regular"  # or 'Night'
    all_routes_single = requests.get(
        f"https://api.tfl.gov.uk/Line/{line}/Route/Sequence/all?serviceTypes={service_type}"
    )
...
async def _get_stops_on_a_line(lines_to_check):
    ...
    for transport in lines_to_check:
        stops_on_line = requests.get(
            f"https://api.tfl.gov.uk/Line/{transport}/StopPoints"
        )
        logger.debug(
            "stops_on_line: %s",
...
async def _get_tube_status_update():
    ...
    # Construct a DataFrame from the status dictionary and normalize columns


async def _next_train_or_bus(tube_and_bus_stops):
    ...
    for station, line in tube_and_bus_stops.values():
        schedule_raw = requests.get(
            f"https://api.tfl.gov.uk/Line/{line}/Arrivals/{station}"
        )
        schedule_neat = json.loads(schedule_raw.text)
        station_and_direction = (
            f"{schedule_neat[0]['stationName']} {schedule_neat[0]['platformName']}"
        )
    ...
    for y in next_transport_dict.keys():
        for z in range(len(next_transport_dict[y])):
            ...
            if next_transport_dict[y][z]["currentLocation"]:
                new_row["currentLocation"] = next_transport_dict[y][z][
                    "currentLocation"
                ]
    ...
    # Format expected arrival as HH:MM:SS string for display without using .dt accessor
    eta_dashboard_df["expectedArrival"] = eta_dashboard_df["expectedArrival"].apply(
        lambda x: x.strftime("%H:%M:%S") if hasattr(x, "strftime") else ""
    )
...
if __name__ == "__main__":
    ...
    next_tube_and_bus = asyncio.run(_next_train_or_bus(tube_and_bus_stops))

    # startup script; nothing to print here
